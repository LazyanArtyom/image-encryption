...
/// CLogicEncoderRGB
/////////////////////////////////
CLogicEncoderRGB::CLogicEncoderRGB(size_t nColNum, size_t nRowNum)
    : m_nColNum(nColNum * 8), m_nRowNum(nRowNum)
{
    m_pLogicForDec = new CLogic(m_nColNum, m_nRowNum, 2);
...
CLogicEncoderRGB::~CLogicEncoderRGB()
{
    if(m_pLogicForDec != nullptr)
    {
        delete m_pLogicForDec;
...
std::vector<std::vector<int>> CLogicEncoderRGB::generateKey()
{
    std::vector<std::vector<int>> result;
    result.resize(m_nRowNum);
    for(auto& row : result)
...
void CLogicEncoderRGB::setImage(std::vector<std::vector<int> > matrix)
{
    for(size_t i = 0; i < matrix.size(); ++i)
    {
        for(size_t j = 0; j < matrix[0].size(); ++j)
        {
            ...
            while(k > 0)
            {
                if(val != 0)
                {
                    m_arrImage[i][j * 8 + k - 1] = val % 2;
                    val /= 2;
                }else
                {
                    m_arrImage[i][j * 8 + k - 1] = 0;
                }
                k--;
            }
        }
...
std::vector<std::vector<int>> CLogicEncoderRGB::getImage()
{
    return m_arrImage;
}

std::vector<std::vector<int>> CLogicEncoderRGB::encode()
{
    for(size_t i = 0; i < m_nRowNum; ++i)
    {
        for(size_t j = 0; j < m_nColNum; ++j)
    ...
    for(size_t i = 0; i < matrix.size(); ++i)
    {
        ...
        for(size_t j = 0; j < matrix[0].size(); ++j)
        {
            ...
            while(k > 0)
            {
                int val = m_arrImage[i][j * 8 + k - 1] > 2 ? 1 : 0;
                matrix[i][j] += val * std::pow(2, 8 - k);
                k--;
            }
        }
    }
...
void CLogicEncoderRGB::getDecodedAndFinished(std::vector<std::vector<int> > &matrix)
{
    if(m_bFirstDecodeReq)
    {
        for(size_t i = 0; i < m_nRowNum; ++i)
